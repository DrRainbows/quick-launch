...
function generateStage(params) {
  const {
    ...
    propMass: initialPropMass, targetDeltaV, numEngines, fairingDia, payloadAbove,
    maxAccelG, tankMaterial,
  } = params;

  let propMass = initialPropMass;
  ...
  let actualTotalThrustVac = propellant === "SOLID"
    ? engines.thrustVac
    : engines.thrustVac * numEngines;
  let actualTotalThrustSL = propellant === "SOLID"
    ? engines.thrustSL
  ...
  // Total engine mass
  let totalEngineMass = propellant === "SOLID"
    ? engines.dryMass
  ...
  let avionicsMass = isUpperStage ? randRange(80, 200) : randRange(150, 400);
  let tvsMass = engines.gimbalAngle > 0 ? numEngines * randRange(20, 60) : 0;
  ...
  let interstageMass = stageIndex < totalStages - 1
    ? Math.PI * fairingDia * interstageLength * interstageThick * interstageMat.density * 1.3
    : 0;

  // Thrust structure (updated if thrust scales)
  let thrustStructMass = actualTotalThrustVac * 0.0008;

  // Initial dry mass from component budget
  let dryMass = totalEngineMass + tankStructMass + avionicsMass + tvsMass
    + pressurantMass + interstageMass + thrustStructMass;

  // Reconcile structure to target mass fraction (pattern language convergence)
  if (dryMass > dryTarget && dryTarget > 0) {
    const scale = dryTarget / dryMass;
    tankStructMass *= scale;
    avionicsMass *= scale;
    tvsMass *= scale;
    pressurantMass *= scale;
    interstageMass *= scale;
    thrustStructMass *= scale;
    if (propellant !== "SOLID") totalEngineMass *= scale;
    dryMass = totalEngineMass + tankStructMass + avionicsMass + tvsMass
      + pressurantMass + interstageMass + thrustStructMass;
  }

  // Stage delta-v (Tsiolkovsky) — reconcile propellant to hit allocated budget
  const ispForDv = isFirstStage ? lerp(ispSL, ispVac, 0.5) : ispVac;
  if (targetDeltaV && targetDeltaV > 0) {
    const mf = dryMass + payloadAbove;
    const massRatio = Math.exp(targetDeltaV / (ispForDv * PHYS.g0));
    const requiredProp = mf * (massRatio - 1);
    if (requiredProp > propMass) {
      propMass = requiredProp;
      if (propellant !== "SOLID") {
        const maxAccel = maxAccelG * PHYS.g0;
        tanks = sizeTanks(propMass, engines.mixtureRatio, prop, tankMaterial, maxAccel, engines.chamberPressure, fairingDia);
        tankStructMass = tanks.totalStructuralMass;
  ...
  let grossMass = propMass + dryMass;

  // First stage: thrust must lift the full stack; cap acceleration at liftoff and burnout separately
  if (isFirstStage) {
    const liftoffMass = grossMass + payloadAbove;
    const minTwr = 1.3;
    const reqThrustSL = liftoffMass * PHYS.g0 * minTwr;
    const maxThrustAtLiftoff = liftoffMass * maxAccelG * PHYS.g0;
    const maxThrustVac = (dryMass + payloadAbove) * maxAccelG * PHYS.g0;
    let targetThrustSL = Math.max(actualTotalThrustSL, reqThrustSL);
    targetThrustSL = Math.min(targetThrustSL, maxThrustAtLiftoff);
    if (targetThrustSL > actualTotalThrustSL) {
      const scale = targetThrustSL / actualTotalThrustSL;
      actualTotalThrustSL = targetThrustSL;
      actualTotalThrustVac = Math.min(
        actualTotalThrustSL * (engines.thrustVac / engines.thrustSL),
        maxThrustVac
      );
      if (propellant !== "SOLID") {
        engines.thrustSL *= scale;
        engines.thrustVac = actualTotalThrustVac / numEngines;
      }
      thrustStructMass = actualTotalThrustVac * 0.0008;
      dryMass = totalEngineMass + tankStructMass + avionicsMass + tvsMass
        + pressurantMass + interstageMass + thrustStructMass;
      grossMass = propMass + dryMass;
    }
  }

  // Reconcile propellant again after thrust/dry-mass changes
  if (targetDeltaV && targetDeltaV > 0) {
    const mf = dryMass + payloadAbove;
    const requiredProp = mf * (Math.exp(targetDeltaV / (ispForDv * PHYS.g0)) - 1);
    if (requiredProp > propMass) {
      propMass = requiredProp;
      grossMass = propMass + dryMass;
    }
  ...
  // Cap vacuum thrust so burnout acceleration respects maxAccelG (all stages)
  let maxAccelG_actual = actualTotalThrustVac / (dryMass + payloadAbove) / PHYS.g0;
  if (maxAccelG_actual > maxAccelG && actualTotalThrustVac > 0) {
    const cappedVac = (dryMass + payloadAbove) * maxAccelG * PHYS.g0;
    const vacScale = cappedVac / actualTotalThrustVac;
    actualTotalThrustVac = cappedVac;
    actualTotalThrustSL *= vacScale;
    if (propellant !== "SOLID") {
      engines.thrustVac *= vacScale;
      engines.thrustSL *= vacScale;
    }
    thrustStructMass = actualTotalThrustVac * 0.0008;
    dryMass = totalEngineMass + tankStructMass + avionicsMass + tvsMass
      + pressurantMass + interstageMass + thrustStructMass;
    grossMass = propMass + dryMass;
    maxAccelG_actual = maxAccelG;
  }

  // Engine-out capability
  const engineOutCapable = numEngines >= 2 && propellant !== "SOLID";
  const liftoffMassForTwr = grossMass + payloadAbove;
  const engineOutTWR = engineOutCapable
    ? (actualTotalThrustSL * (numEngines - 1) / numEngines) / (liftoffMassForTwr * PHYS.g0)
    : 0;
...
function validateRocket(rocket) {
  ...
  // 2. First stage TWR > 1.0 (include strap-on boosters when present)
  const s1 = rocket.stages[0];
  const liftoffMass = rocket.totalMass;
  const liftoffThrust = rocket.performance?.liftoffThrust || s1.totalThrustSL;
  const liftoffTWR = liftoffThrust / (liftoffMass * PHYS.g0);
  if (liftoffTWR < 1.05) {
...
function allocatePropellant(requiredDv, payloadMass, stageConfigs, numStages) {
  ...
  // Normalize with first-stage bias for ascent/gravity-turn losses
  const dvTotal = dvSplit.reduce((s, v) => s + v, 0);
  dvSplit = dvSplit.map((v, i) => {
    const bias = i === 0 ? 1.12 : (i === numStages - 1 ? 0.94 : 1.0);
    return v * bias * requiredDv / dvTotal;
  });

  // Add 8% margin on total budget
  dvSplit = dvSplit.map(v => v * 1.08);
  ...
  return {
    propMasses: propMasses.map(m => roundTo(m, 0)),
    dvSplit: dvSplit.map(v => roundTo(v, 1)),
  };
}
...
function generateRocket(latitude, targetOrbitClass, options = {}) {
  ...
  const { propMasses, dvSplit } = allocatePropellant(requiredDeltaV, payloadMass, stageConfigs, numStages);
  ...
  // Fairing and booster pad mass aren't in stage-0 thrust sizing — reconcile liftoff TWR
  {
    const s0 = stageSpecs[0];
    const boosterThrustSL = boosters?.totalThrustSL || 0;
    const minLiftoffThrust = totalMass * PHYS.g0 * 1.05;
    const liftoffThrust = s0.totalThrustSL + boosterThrustSL;
    if (liftoffThrust < minLiftoffThrust) {
      const maxCoreSL = totalMass * s0.maxAccelG * PHYS.g0 - boosterThrustSL;
      const targetCoreSL = Math.min(
        Math.max(s0.totalThrustSL, minLiftoffThrust - boosterThrustSL),
        maxCoreSL
      );
      if (targetCoreSL > s0.totalThrustSL) {
        const scale = targetCoreSL / s0.totalThrustSL;
        s0.totalThrustSL = roundTo(targetCoreSL, 0);
        const burnoutMassAbove = totalMass - s0.grossMass;
        const maxVac = burnoutMassAbove * s0.maxAccelG * PHYS.g0;
        s0.totalThrustVac = roundTo(Math.min(s0.totalThrustVac * scale, maxVac), 0);
        if (s0.engine && s0.propellant !== 'SOLID') {
          s0.engine.thrustSL *= scale;
          s0.engine.thrustVac = s0.totalThrustVac / s0.engineCount;
        }
        s0.maxAccelG = roundTo(s0.totalThrustVac / (burnoutMassAbove * PHYS.g0), 2);
      }
    }
  }
...
/**
 * Generate a rocket that passes validateRocket(). Retries with varied payload
 * mass until the pattern language converges on a flyable vehicle.
 *
 * @throws {Error} if no valid vehicle within maxAttempts
 */
function generateViableRocket(latitude, targetOrbitClass, options = {}) {
  const maxAttempts = options.maxAttempts || 48;
  let best = null;
  let bestMargin = -Infinity;

  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const payloadMass = options.payloadMass
      || Math.round(randRange(3000, 10000));
    const rocket = generateRocket(latitude, targetOrbitClass, { ...options, payloadMass });

    if (rocket.validation.valid) return rocket;

    const margin = rocket.validation.totalDeltaV / rocket.mission.requiredDeltaV;
  ...
  if (best) {
    throw new Error(
      `No valid rocket in ${maxAttempts} attempts. Best margin: ${(bestMargin * 100).toFixed(1)}% ` +
      `(${best.validation.totalDeltaV}/${best.mission.requiredDeltaV} m/s). ` +
      `Warnings: ${best.validation.warnings.join('; ')}`
    );
  }
  throw new Error(`Rocket generation failed at lat=${latitude} orbit=${targetOrbitClass}`);
}
